...
**Состояние:** v0-2-9 [опубликована и активирована локально](docs/releases/v0-2-9.md). Она сокращает маршрут проверок и очищает прежние выпуски после успешного обновления. `approve` и `push` разделены. Текущую установленную версию проверяет `global-status`. Пилоты целевых проектов отложены. Действующий маршрут — в [глобальном контракте](docs/workflow/GLOBAL.md).
