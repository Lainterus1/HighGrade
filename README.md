...
**Состояние:** Поставка `v0-2-13` / CLI `0.2.13` активирована из проверенного коммита; [запись выпуска](docs/releases/v0-2-13.md). Нативный JSON v2: номера, прогресс и человеческая приёмка. Старые OpenSpec-материалы сохранены. Авторский текст следует языку проекта. Установленную версию показывает `global-status`; маршрут — в [GLOBAL](docs/workflow/GLOBAL.md).
